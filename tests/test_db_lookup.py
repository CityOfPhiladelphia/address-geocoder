...
def test_insert_to_tmp_tables(monkeypatch):
    ...
    def write_fake_database(
        self, table_name, connection, engine, if_table_exists, **kwargs
    ):
        calls.append(
            {
                "nrows": self.height,
    ...
    assert all([call["table"] == "pg_temp.tmp_geo_append_table" for call in calls])
...
def test_append_calls_insert_and_returns_join_result(monkeypatch):
    ...
    monkeypatch.setattr(
        db_lookup.pl, "read_database", lambda query, connection: joined_df
    )
